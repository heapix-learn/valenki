...
!function (t, e) {
	...
	return function (t) {
		...
		"use strict";
		n.r(e);
		var i = {throttleLimit: 50, loopCheckTimeout: 1e3, loopCheckMaxCalls: 10},
			r = function () {
				var t = !1;
				try {
			...
			l = {color: "#666", fontSize: "14px", padding: "10px 0"}, d = {
				mode: "development",
		...
		var S = f({
			...
			mounted: function () {
				...
				this.$watch("forceUseInfiniteWrapper", function () {
					...
					this.status === s.READY && (t && t.constructor === Event && k(this.$el) ? v.throttle(this.attemptLoad) : this.attemptLoad())
				}.bind(this), setTimeout(this.scrollHandler, 1), this.scrollParent.addEventListener("scroll", this.scrollHandler, r), this.$on("$InfiniteLoading:loaded", function (e) {
